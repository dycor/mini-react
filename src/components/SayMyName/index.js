...
function prop_access(path,obj) {
  ...
  for (let i = 0; i < aPath.length; i++) {
    if (object[aPath[i]] === undefined) {
      console.log(`${aPath.slice(0, i + 1).join('.')} don't exist`);
      return;
    }
...
String.prototype.interpolate = function(result){
  return this.replace(/\{\{([^}]+)\}\}/g, (dummy, v) => prop_access(v,result));
};
...
export class SayMyName extends Martine.Component {
  ...
  constructor(props) {
    super(props);
    this.state = { name :'' , animal : { test : 'nan'}};
  }
    ...
    return  Martine.createElement('div', null,
        Martine.createElement('div', {class:'pure-g', style: 'display:flex; align-items: center; justify-content: center; '},
            Martine.createElement('div',{class:'pure-u-1-5', style: 'background-color: #fff;padding: 20px;color: #48AFE2;border-radius: 25px;box-shadow: 10px 3px 10px 0px rgba(0,0,0,0.75)'},
                ...
                Martine.createElement('p', null,'My name is {{name}}'.interpolate(this.state))
            ),
        ...
        Martine.createElement('a', {class:'pure-button pure-button-primary', href:'/props', style: 'margin: 0 auto;width: 20%;display: block;'}, 'Introduction aux props')
    );
  }
}
